...
# installed = "/home/zhouhao/data_fast/android_9/out/target/product/generic_x86/installed-files.txt"
...
for so_name in soname2sopath.keys():
    ...
    command = ("llvm-link --only-needed %s" % (ll_path))
    for so_dep in so_deps:
        if not (so_dep in binder_sonames) and not (so_dep == "libutils.so") and not (so_dep == "libc.so"):
            print "  [IGNORE] " + so_dep
            continue
